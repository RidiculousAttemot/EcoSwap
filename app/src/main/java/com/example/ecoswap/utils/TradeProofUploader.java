...
/**
 * Handles compression and upload of trade proof photos to Supabase storage.
 */
public final class TradeProofUploader {

    private static final String TAG = "TradeProofUploader";
    private static final String DEFAULT_BUCKET = "trade-proofs";
    private static final int MAX_DIMENSION = 1080;
    private static final int JPEG_QUALITY = 82;

    private TradeProofUploader() {
        // Utility class
    }
    ...
    public static void upload(Context context,
        ...
        if (tradeId == null || tradeId.isEmpty()) {
            callback.onUploadError("Missing trade id");
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                String objectPath = userId + "/trade_" + tradeId + "_" + UUID.randomUUID() + ".jpg";
                String bucketName = resolveBucketName();
                Log.d(TAG, "Uploading trade proof trade=" + tradeId
                        + " user=" + userId
                        + " bucket=" + bucketName
                        + " path=" + objectPath
                ...
                supabaseClient.uploadFile(bucketName, objectPath, imageBytes, new SupabaseClient.OnStorageCallback() {
                    ...
                    public void onSuccess(String url) {
                        Log.d(TAG, "Trade proof upload success trade=" + tradeId + " url=" + url);
                        mainHandler.post(() -> callback.onUploadSuccess(url));
                    }

                    @Override
                    public void onError(String error) {
                        Log.e(TAG, "Trade proof upload failed trade=" + tradeId + ": " + error);
                        String message = error != null ? error : "Upload failed";
                        mainHandler.post(() -> callback.onUploadError(message));
                    }
                });
            } catch (Exception e) {
                Log.e(TAG, "Trade proof upload failed", e);
    ...
    private static String resolveBucketName() {
        String configuredBucket = BuildConfig.SUPABASE_PROOFS_BUCKET;
        if (configuredBucket == null || configuredBucket.trim().isEmpty()) {
            Log.w(TAG, "SUPABASE_PROOFS_BUCKET missing. Falling back to " + DEFAULT_BUCKET);
